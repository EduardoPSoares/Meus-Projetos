...
async function populateServerListings(){
    ...
    for(const serv of servers){
        const iconSrc = serv.rawServer.icon && /^https?:\/\//i.test(serv.rawServer.icon) ? escapeHtml(serv.rawServer.icon) : ''
        htmlString += `<button class="serverListing" servid="${escapeHtml(serv.rawServer.id)}" ${serv.rawServer.id === giaSel ? 'selected' : ''}>
            ${iconSrc ? `<img class="serverListingImg" src="${iconSrc}"/>` : ''}
...
function populateAccountListings(){
    ...
    for(let i=0; i<accounts.length; i++){
        htmlString += `<button class="accountListing" uuid="${escapeHtml(accounts[i].uuid)}" ${i===0 ? 'selected' : ''}>
            <img src="https://mc-heads.net/head/${encodeURIComponent(accounts[i].uuid)}/40">
