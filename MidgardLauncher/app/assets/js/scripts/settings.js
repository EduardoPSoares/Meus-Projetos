        ...
        if(typeof sFn === 'function'){
            if(v.tagName === 'INPUT'){
                if(v.type === 'number' || v.type === 'text'){
                    ...
                    if(cVal === 'AudioSensitivity'){
                        const bgMusic = document.getElementById('bg-music')
                        if(bgMusic) {
                            bgMusic.volume = v.checked ? 0.04 : 0.15
                            const musicSliderEl = document.getElementById('musicVolumeSlider')
                            if(musicSliderEl) {
                                musicSliderEl.value = v.checked ? 4 : 15
                            }
                        }
                    }
                }
...
document.getElementById('settingsAddOfflineAccount').onclick = (e) => {
    ...
    setTimeout(() => {
        ...
        if(overlayDesc && !document.getElementById('offlineUsernameInput')) {
            const inputHtml = '<br><input id="offlineUsernameInput" type="text" placeholder="' + Lang.queryJS('loginOptions.offlineUsernamePlaceholder') + '" style="width:60%;padding:8px 12px;margin-top:12px;border:2px solid #555;border-radius:4px;background:#333;color:#fff;font-size:14px;outline:none;text-align:center;" maxlength="16" autofocus>'
            overlayDesc.innerHTML += inputHtml
            const inp = document.getElementById('offlineUsernameInput')
            if(inp) inp.focus()
        }
    }, 600)
}
    ...
    if (arguments_[0] === MSFT_REPLY_TYPE.ERROR) {
        ...
        if (Object.prototype.hasOwnProperty.call(queryMap, 'error')) {
            switchView(getCurrentView(), viewOnClose, 500, 500, () => {
                ...
                setOverlayContent(
                    escapeHtmlSettings(error),
                    escapeHtmlSettings(errorDesc),
                    Lang.queryJS('settings.msftLogin.okButton')
                )
                setOverlayHandler(() => {
                    toggleOverlay(false)
...
function populateAuthAccounts(){
    ...
    authKeys.forEach((val) => {
        ...
        const accHtml = `<div class="settingsAuthAccount" uuid="${escapeHtmlSettings(acc.uuid)}">
            <div class="settingsAuthAccountLeft">
                <img class="settingsAuthAccountImage" alt="${escapeHtmlSettings(acc.displayName)}" src="https://mc-heads.net/body/${encodeURIComponent(acc.uuid)}/60">
            </div>
            <div class="settingsAuthAccountRight">
                <div class="settingsAuthAccountDetails">
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">${Lang.queryJS('settings.authAccountPopulate.username')}</div>
                        <div class="settingsAuthAccountDetailValue">${escapeHtmlSettings(acc.displayName)}</div>
                    </div>
                    <div class="settingsAuthAccountDetailPane">
                        <div class="settingsAuthAccountDetailTitle">${Lang.queryJS('settings.authAccountPopulate.uuid')}</div>
                        <div class="settingsAuthAccountDetailValue">${escapeHtmlSettings(acc.uuid)}</div>
                    </div>
                </div>
    ...
    if(document.getElementById('settingsCurrentOfflineAccounts')) {
...
async function resolveDropinModsForUI(){
    ...
    CACHE_BLOCKED_PATTERNS = serv.rawServer.blockedMods || []
    CACHE_DROPIN_MODS = DropinModUtil.scanForDropinMods(CACHE_SETTINGS_MODS_DIR, serv.rawServer.minecraftVersion, CACHE_BLOCKED_PATTERNS)

    let dropinMods = ''

    for(const dropin of CACHE_DROPIN_MODS){
        const isBlocked = dropin.blocked
        dropinMods += `<div id="${dropin.fullName}" class="settingsBaseMod settingsDropinMod${isBlocked ? ' settingsBlockedMod' : ''}" ${!dropin.disabled && !isBlocked ? 'enabled' : ''}>
                    <div class="settingsModContent">
                        <div class="settingsModMainWrapper">
                            <div class="settingsModStatus"></div>
                            <div class="settingsModDetails">
                                <span class="settingsModName">${dropin.name}${isBlocked ? ' <span class="settingsBlockedLabel">⛔ ' + Lang.queryJS('settings.dropinMods.blockedLabel') + '</span>' : ''}</span>
                        ...
                        <label class="toggleSwitch"${isBlocked ? ' title="' + Lang.queryJS('settings.dropinMods.blockedTooltip') + '"' : ''}>
                            <input type="checkbox" formod="${dropin.fullName}" dropin ${!dropin.disabled && !isBlocked ? 'checked' : ''} ${isBlocked ? 'disabled' : ''}>
                            <span class="toggleSwitchSlider"></span>
                        </label>
                    </div>
                </div>`
    ...
    Array.from(sEls).map((v, index, arr) => {
        v.onclick = async () => {
            ...
            if(res){
                ...
                setOverlayContent(
                    Lang.queryJS('settings.dropinMods.deleteFailedTitle', { fullName: escapeHtmlSettings(fullName) }),
...
function bindDropinModFileSystemButton(){
    ...
    fsBtn.ondrop = async e => {
        ...
        const rejected = DropinModUtil.addDropinMods(e.dataTransfer.files, CACHE_SETTINGS_MODS_DIR, CACHE_BLOCKED_PATTERNS)
        if(rejected.length > 0) {
            setOverlayContent(
                Lang.queryJS('settings.dropinMods.blockedTitle'),
                Lang.queryJS('settings.dropinMods.blockedMessage', { mods: rejected.map(escapeHtmlSettings).join(', ') }),
                Lang.queryJS('settings.dropinMods.okButton')
            )
            setOverlayHandler(null)
            toggleOverlay(true)
                ...
                DropinModUtil.toggleDropinMod(CACHE_SETTINGS_MODS_DIR, dropin.fullName, dropinUIEnabled).catch(err => {
                    if(!isOverlayVisible()){
                        setOverlayContent(
                            ...
                            escapeHtmlSettings(err.message),
                            Lang.queryJS('settings.dropinMods.okButton')
                        )
                        setOverlayHandler(null)
                        toggleOverlay(true)
...
async function loadSelectedServerOnModsTab(){
    ...
    for(const el of document.getElementsByClassName('settingsSelServContent')) {
        el.innerHTML = `
            ${serv.rawServer.icon ? `<img class="serverListingImg" src="${serv.rawServer.icon}"/>` : ''}
...
function populateReleaseNotes(){
    ipcRenderer.invoke('fetchReleases').then(releases => {
        if(!releases) {
        ...
        for(const rel of releases){
            if(rel.tag_name === version){
                settingsAboutChangelogTitle.textContent = rel.name || rel.tag_name
                settingsAboutChangelogText.textContent = rel.body || ''
                if(typeof rel.html_url === 'string' && rel.html_url.startsWith('https://')) {
                    settingsAboutChangelogButton.href = rel.html_url
                }
                break
            }
        }
 ...
 */
function settingsUpdateButtonStatus(text, disabled = false, handler = null, pulseClass = null){
    settingsUpdateActionButton.innerHTML = text
    settingsUpdateActionButton.disabled = disabled
    settingsUpdateActionButton.classList.remove('pulse-checking', 'pulse-update-ready')
    if(pulseClass){
        settingsUpdateActionButton.classList.add(pulseClass)
    }
    if(handler != null){
...
function populateSettingsUpdateInformation(data){
    if(data != null){
        ...
        if(process.platform === 'darwin'){
            ...
            settingsUpdateButtonStatus(Lang.queryJS('settings.updates.downloadingButton'), true, null, 'pulse-checking')
        }
    } else {
        ...
        settingsUpdateButtonStatus(Lang.queryJS('settings.updates.checkForUpdatesButton'), false, () => {
            if(!isDev){
                ...
                settingsUpdateButtonStatus(Lang.queryJS('settings.updates.checkingForUpdatesButton'), true, null, 'pulse-checking')
            }
        })
        // Fetch and display current version release notes
        populateUpdateReleaseNotes()
    }
}

function populateUpdateReleaseNotes(){
    ipcRenderer.invoke('fetchReleases').then(releases => {
        ...
        settingsUpdateChangelogTitle.innerHTML = latest.name || latest.tag_name
        settingsUpdateChangelogText.innerHTML = latest.body || ''
        settingsUpdateChangelogCont.style.display = null
    }).catch(() => {
        settingsUpdateChangelogCont.style.display = 'none'
    })
