...
try {
    ...
    FileLog = FileLogger.getLogger('AuthManager')
} catch(_) {
...
exports.addMojangAccount = async function(username, password) {
    try {
        FileLog.info('Attempting Mojang login for:', username)
        ...
        log.error(err)
        FileLog.error('Mojang authentication error:', err)
...
async function fullMicrosoftAuthFlow(entryCode, authMode) {
    try {
        FileLog.info('Starting Microsoft auth flow. Mode:', authMode)
        ...
        log.error(err)
        FileLog.error('Microsoft auth flow error:', err)
...
exports.addMicrosoftAccount = async function(authCode) {
    ...
    const xuid = fullAuth.xsts?.DisplayClaims?.xui?.[0]?.uhs || ''
...
exports.removeMojangAccount = async function(uuid){
    try {
        FileLog.info('Removing Mojang account:', uuid)
        const authAcc = ConfigManager.getAuthAccount(uuid)
        const response = await MojangRestAPI.invalidate(authAcc.accessToken, ConfigManager.getClientToken())
        if(response.responseStatus === RestResponseStatus.SUCCESS) {
            ConfigManager.removeAuthAccount(uuid)
            ConfigManager.save()
            FileLog.info('Mojang account removed successfully:', uuid)
            return Promise.resolve()
        } else {
            log.error('Error while removing account', response.error)
            FileLog.error('Error removing Mojang account:', response.error)
...
exports.removeMicrosoftAccount = async function(uuid){
    try {
        FileLog.info('Removing Microsoft account:', uuid)
        ConfigManager.removeAuthAccount(uuid)
        ConfigManager.save()
        FileLog.info('Microsoft account removed successfully:', uuid)
        return Promise.resolve()
    } catch (err){
        log.error('Error while removing account', err)
        FileLog.error('Error removing Microsoft account:', err)
        return Promise.reject(err)
    }
}
...
async function validateSelectedMojangAccount(){
    ...
    FileLog.debug('Validating Mojang account:', current.uuid)
    ...
    if(response.responseStatus === RestResponseStatus.SUCCESS) {
        ...
        log.error('Mojang validation request failed:', response.error || response.responseStatus)
        FileLog.error('Mojang account validation request failed')
        return false
    }
}
...
async function validateSelectedMicrosoftAccount(){
    const current = ConfigManager.getSelectedAccount()
    const now = new Date().getTime()
    FileLog.debug('Validating Microsoft account:', current.uuid)
    ...
    if(msExpired) {
        ...
        try {
            ...
            const xuid = res.xsts?.DisplayClaims?.xui?.[0]?.uhs || ''
...
/**
 * Add an offline account. No authentication is performed.
 * 
...
exports.addOfflineAccount = function(username) {
    ...
    const ret = ConfigManager.addOfflineAuthAccount(formattedUuid, username)
    ConfigManager.save()
    return ret
}
