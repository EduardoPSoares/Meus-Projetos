...
try {
    ...
    FileLog = FileLogger.getLogger('ConfigManager')
} catch(_) {
...
const dataPath = path.join(sysRoot, '.midgardlauncher')
...
const DEFAULT_CONFIG = {
    settings: {
        ...
        performance: {
            ...
            performancePromptShown: false,
            appliedPerformanceTier: null,
            appliedAudioSensitivity: null
        }
    },
    newsCache: {
        date: null,
...
exports.save = function(){
    try {
        ...
        FileLog.error('Failed to save configuration file:', err)
    }
}
...
exports.load = function(){
    ...
    if(doLoad){
        ...
        try {
            ...
            FileLog.error('Configuration file malformed or corrupt:', err)
            FileLog.info('Generating new default configuration file.')
            fs.ensureDirSync(path.join(configPath, '..'))
            config = DEFAULT_CONFIG
            exports.save()
        }
    ...
    FileLog.info('Configuration loaded successfully. DataDir:', dataPath)
}
 ...
 * @returns {Object} The authenticated account object created by this action.
 */
exports.updateMicrosoftAuthAccount = function(uuid, accessToken, msAccessToken, msRefreshToken, msExpires, mcExpires, xuid) {
    config.authenticationDatabase[uuid].accessToken = accessToken
 ...
 * @returns {Object} The authenticated account object created by this action.
 */
exports.addMicrosoftAuthAccount = function(uuid, accessToken, name, mcExpires, msAccessToken, msRefreshToken, msExpires, xuid) {
    config.selectedAccount = uuid
    config.authenticationDatabase[uuid] = {
        type: 'microsoft',
        accessToken,
...
/**
 * Adds an offline (pirate) account to the database.
 * 
 * @param {string} uuid The generated uuid.
 * @param {string} displayName The username.
 * 
 * @returns {Object} The authenticated account object created by this action.
 */
...
exports.getAudioSensitivity = function(def = false){
    return !def ? config.settings.accessibility.audioSensitivity : DEFAULT_CONFIG.settings.accessibility.audioSensitivity
}

exports.setAudioSensitivity = function(audioSensitivity){
    config.settings.accessibility.audioSensitivity = audioSensitivity
}

exports.isAccessibilityPromptShown = function(){
    return config.settings.accessibility.accessibilityPromptShown
}

exports.setAccessibilityPromptShown = function(shown){
    config.settings.accessibility.accessibilityPromptShown = shown
}

// Performance Settings

function ensurePerformanceConfig(){
    if(!config.settings.performance){
        config.settings.performance = Object.assign({}, DEFAULT_CONFIG.settings.performance)
    }
}

exports.getPerformanceTier = function(def = false){
    ensurePerformanceConfig()
    return !def ? config.settings.performance.performanceTier : DEFAULT_CONFIG.settings.performance.performanceTier
}

exports.setPerformanceTier = function(tier){
    ensurePerformanceConfig()
    config.settings.performance.performanceTier = tier
}

exports.isPerformancePromptShown = function(){
    ensurePerformanceConfig()
    return config.settings.performance.performancePromptShown
}

exports.setPerformancePromptShown = function(shown){
    ensurePerformanceConfig()
    config.settings.performance.performancePromptShown = shown
}

exports.getAppliedPerformanceTier = function(){
    ensurePerformanceConfig()
    return config.settings.performance.appliedPerformanceTier || null
}

exports.setAppliedPerformanceTier = function(tier){
    ensurePerformanceConfig()
    config.settings.performance.appliedPerformanceTier = tier
}

exports.getAppliedAudioSensitivity = function(){
    ensurePerformanceConfig()
    return config.settings.performance.appliedAudioSensitivity
}

exports.setAppliedAudioSensitivity = function(value){
    ensurePerformanceConfig()
    config.settings.performance.appliedAudioSensitivity = value
}