...
public class MidgardStaminaCondition implements IEntityCondition {

    private final String valueRange;
    private final boolean percentage;

    public MidgardStaminaCondition(MythicLineConfig config) {
    ...
    public boolean check(AbstractEntity entity) {
        ...
        double current = combatData.getCurrentStamina();
        
        if (percentage) {
            CoreAttributeData attrData = profile.getOrCreateData(CoreAttributeData.class);
            AttributeInstance maxAttr = attrData.getInstance(CombatAttributes.MAX_STAMINA);
    ...
    private boolean compare(double value, String range) {
        try {
            if (range.startsWith(">=")) {
                double val = Double.parseDouble(range.substring(2));
                return value >= val;
                ...
                double val = Double.parseDouble(range.substring(1));
                return Math.abs(value - val) < 0.01;
            } else {
